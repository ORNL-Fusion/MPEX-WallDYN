...
% calculate surface area averaged values for walldyn
for i=1:nSurfaces
    ti(i) = sum(ti_geom(surf_ind_cell{i}).*area(surf_ind_cell{i}))./area_surfs(i);
    te(i) = sum(te_geom(surf_ind_cell{i}).*area(surf_ind_cell{i}))./area_surfs(i);
    flux(i) = sum(flux_geom(surf_ind_cell{i}).*area(surf_ind_cell{i}))./area_surfs(i);
    potential(i) = sum(potential_geom(surf_ind_cell{i}).*area(surf_ind_cell{i}))./area_surfs(i);
    angle(i) = sum(angle_geom(surf_ind_cell{i}).*area(surf_ind_cell{i}))./area_surfs(i);
end
...
figure
patch(transpose(X),transpose(Y),transpose(Z),angle_geom,'FaceAlpha',1,'EdgeAlpha', 0.3)%,impacts(surface)
colorbar
title('ProtoMPEX Geometry B-field angle [degrees]')
xlabel('x [m]')
ylabel('y [m]')
zlabel('z [m]')
set(gca,'fontsize',13)
