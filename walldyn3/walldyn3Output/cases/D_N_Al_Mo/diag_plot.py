...
def main():
    ...
    ErosionFlux_of_N.Plot2D(axes, 'Time', 'N_90', yscale=scalefunc(ErosionFlux_of_N, 'N_90'), label='Ero-Flux')
    Gamma_of_N.Plot2D(axes, 'Time', 'N_90', yscale=scalefunc(Gamma_of_N, 'N_90'), label='Influx')
    ReflectedFlux_of_N.Plot2D(axes, 'Time', 'N_90', yscale=scalefunc(ReflectedFlux_of_N, 'N_90'), label='Refl-Flux')
    RYield_of_n.Plot2D(axes, 'Time', 'N_1_90', yscale=scalefunc(RYield_of_n, 'N_1_90')*80, yoff=-79.6, label='Refl-Yield', linewidth=2.0)
    SYieldBy_of_n_n.Plot2D(axes, 'Time', 'n_n_1_90', yscale=scalefunc(SYieldBy_of_n_n, 'n_n_1_90'), label='Sputter-Yield')
    Conc_of_N.Plot2D(axes, 'Time', 'N_90', yscale=scalefunc(Conc_of_N, 'N_90')*100, yoff=-99.4, label='Conc.')
    TotalSource_of_N.Plot2D(axes, 'Time', 'N_90', yscale=scalefunc(TotalSource_of_N, 'N_90'), label='Tot. Src.')
